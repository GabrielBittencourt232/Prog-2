...
class Veiculo implements Repositorio {
    ...
    [cite_start]// [Conceito POO: Encapsulamento - Atributos 'private' para proteger o estado interno e garantir controle via Getters/Setters] [cite: 2280]
    private ?int $id = null;
    ...
    [cite_start]// [Conceito POO: Construtor - Inicializa o objeto Veiculo e garante um estado inicial válido] [cite: 2187, 2219]
    public function __construct(string $marca, string $modelo, int $ano, float $preco, string $cor, ?int $id = null) {
        $this->id = $id;
        $this->setMarca($marca);
    ...
    // --- Getters (Acessores) ---
    [cite_start]// [Conceito POO: Getters - Permitem leitura controlada dos atributos privados] [cite: 2316]
    ...
    [cite_start]// [Conceito POO: Setters - Permitem modificação dos atributos privados com validação] [cite: 2324, 2328]
    ...
    public function salvar(object $obj): bool {
        ...
        if ($obj->getId() !== null) {
            $sql = "UPDATE veiculos SET marca = :marca, modelo = :modelo, ano = :ano, preco = :preco, cor = :cor WHERE id = :id";
            $this->registrarLog("Atualizando veículo ID: {$obj->getId()}");
        } else {
            $sql = "INSERT INTO veiculos (marca, modelo, ano, preco, cor) VALUES (:marca, :modelo, :ano, :preco, :cor)";
            $this->registrarLog("Criando novo veículo.");
        }

        try {
            $stmt = $pdo->prepare($sql);
            [cite_start]// [Utilização das aulas: Prepared Statements - Proteção contra SQL Injection] [cite: 1601, 1603, 1653]
    ...
    public function listar(): array {
        try {
            ...
            [cite_start]// Retorna um array de objetos genéricos, mas poderia ser mapeado para objetos Veiculo se a classe não implementasse Repositorio. [cite: 1679]
